...
class ProductView extends StatefulWidget {
  final Produits productView;
  final String categoryId;
   final FavoriteModel? productModel;
  const ProductView(
      {super.key, required this.productView, required this.categoryId, this.productModel});
...
class _ProductViewState extends State<ProductView> {
  ...
  void _addProductToFavorite() async {
    print("id du produit : ${widget.productView.id}");
    ApiResponse apiResponse = await addProductToFavorite(widget.productView.id);
  ...
  void _removeProductFromFavorite() async {
    ...
    ApiResponse apiResponse =
        await removeProductFromFavorite(widget.productView.id);
  ...
  void _addProductToCart() async {
    ApiResponse apiResponse = await addProductToCart(
        widget.productView.id, quatity, widget.productView.prixVenteArticle);
    if (apiResponse.error == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: const Text("Produit ajouté au panier")));
      setState(() {
        _isFavorite = false;
      });
    } else {
  ...
  Future<void> retrieveArticlesByCategorie() async {
    ApiResponse response =
        await getArticlesByCategorie(widget.productView.categorieId.toString());
    if (response.error == null) {
      Map<String, dynamic> jsonResponse = response.data as Map<String, dynamic>;
  ...
  Widget build(BuildContext context) {
    double price = double.parse(widget.productView.prixVenteArticle.toString());
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: LargeText(
          largeTitle: widget.productView.nomArticle.toString(),
        ),
        /* actions: [
          IconButton(
            onPressed: () {
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          MediumText(
                            middleTitle:
                                'stock: ${widget.productView.stock.toString()}',
